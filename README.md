...
七个角色各司其职，通过明确的输入输出契约通信。每个阶段完成即落盘，因此支持**断点续跑**。
...
| 审稿人 Reviewer | 37 项检查 + 9 阻断 + 证据门禁 | 验收裁决 |
| 作家 Writer | 组装研究报告 | 带置信度标签的综述 |
| 可视化师 Visualizer | 渲染交互式图表（零 CDN） | 概念网络 · 热力图 · 趋势图 |

---

## 统计显著性：为什么不能只看共现次数

光统计「概念 A 和 B 很少一起出现」是不够的，这里有两个陷阱。

**陷阱一：没有显著性判断。** 观测共现比期望少 3 篇，是真实信号还是随机波动？在 100 篇语料里那多半是噪声，在 10000 篇里才是强信号。点估计回答不了这个问题。

**陷阱二：多重比较。** 一次要检验几十个概念组合。即便单个组合的假阳性率控制在 5%，检验 40 个组合后出现至少一次假阳性的概率是 `1-0.95^40 ≈ 87%`。不校正就报「显著」，是探索性研究最常见的翻车点。

本项目对每个候选做 **Fisher 精确检验**（2×2 列联表左尾，检验共现不足），再用 **Benjamini-Hochberg** 方法校正多重比较，并同时报告**提升度**与**优势比** —— p 值回答「是否显著」，效应量回答「差多少」，两者缺一不可。

> **实测价值**：在 MOF CO2 capture 的 34 篇语料上，4 条「零共现」候选**全部未通过显著性检验**。
> 这提醒我们：小语料下的零共现很可能只是采样波动，不能直接当作研究空白。
> 报告会明确标注这一点，而不是把它藏起来。

## 可视化

每次运行额外产出一份**自包含 HTML**（`Reports/visualization.html`），不依赖任何 CDN，双击即可打开，断网也能看：

- **概念共现网络** —— 节点按四个语义类别分簇着色，边的粗细表示跨类别共现强度
- **跨类别热力图** —— 暖色区域即候选空白：两者各自都有研究，却很少被放在一起
- **文献年代分布** —— 逐年产出量，一眼看出领域冷热

力导向布局使用固定随机种子，同一份数据每次渲染结果一致，便于跨批次对比。
...
离线单元测试 **72 项全通过**，覆盖去重指纹、降级链、反证判定、知识库、断点状态、门禁裁决。
